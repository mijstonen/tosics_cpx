...
Other options
 ```note: explonary long arguments are not yet (@2019-0304) implemented, they are mentioned here to ease understanding. that is why they are in braces )```
   ...
   as command argument, for example to copy it or debug it. It makes no sense to combine it with -o.
    ...
    -f    ( --force )
   Force compilation, even no changes where detected. For example, when you want to recompile after editing comments or white space in the source file.
...
are:
    ...
    * scripts/CPX-preproces_hash_compile.sh deals with deep compiler related preprocessing up to compilation.
       * actually CPX-preproces_hash_compile is replaced by CPX-precmdproces_hash_compile.sh which is more advanced.
